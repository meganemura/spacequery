...
export const catalog: Readonly<Record<string, Named>> = {
  ...
  "descendants": named(processQueries.descendants, "Processes that descend from one pid.", ["q"], "Processes", "When you need the processes that descend from one pid."),
  "session-processes": named(reportQueries.sessionProcesses, "Processes that a live session started, directly or through children.", [], "Sessions", "When you need the processes a live session started."),
  "heavy-processes": named(reportQueries.heavyProcesses, "Processes that use the most CPU now, CPU time in total, or memory, with the herdr pane that owns each.", [], "Processes", "When something is slow, hot, or runs away, and you need the process and its pane.", true),
  "pane-load": named(reportQueries.paneLoad, "CPU and memory of each herdr pane, summed over the processes it owns.", [], "Processes", "When you need to choose which pane to close or stop.", true),
