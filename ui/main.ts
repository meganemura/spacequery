...
import { isProviderEnabled, loadConfig, providerSummaries, saveProviderEnabled } from "../core/config.ts";
...
import { browserCatalog, type ProviderToggle } from "./catalog.ts";
...
export async function startUi(args: string[]): Promise<void> {
  ...
  const items = browserCatalog(loadUserQueries(process.env), loadUserProviders(process.env, loaders), preferences);
  const providerToggles: ProviderToggle[] = loaders.map((loader) => ({
    name: loader.name,
    enabled: isProviderEnabled(loader.name, preferences),
    summary: providerSummaries[loader.name] ?? loader.name,
  })).sort((a, b) => a.name.localeCompare(b.name));
  const [{ createElement }, { render }, { Browser }] = await Promise.all([import("react"), import("ink"), import("./app.ts")]);
  const app = render(createElement(Browser, {
    items,
    providers: providerToggles,
    onToggleProvider: (name: string, enabled: boolean) => saveProviderEnabled(process.env, name, enabled),
    notice: preferences.warnings.join(" "),
    mouse: !values["no-mouse"],
    initial: { root, scope: values.scope as "root" | "agents" | "all" | undefined ?? "auto", me: values.me, params: {} },
