...
function usage(userQueries: readonly UserQuery[], userProviders: readonly UserProvider[], env: Readonly<Record<string, string | undefined>>, mode: HelpMode): string {
  ...
  return [
    ...
    "claude-sessions uses status. workflow uses status. runs-in-dir uses status (running, exited).",
    "A runtag orphan stays running, so it does not satisfy --until status=exited.",
